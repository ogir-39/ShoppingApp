...
ProductImage = apps.get_model('catalog', 'ProductImage') # <-- Kéo thêm model ProductImage
...
for cat, name, desc, cost, sell, qty, weight in products_data:
    # Bỏ trường thumbnail ra khỏi Product
    ...
    # Tạo ảnh thumbnail riêng ở bảng ProductImage
    if created:
        ProductImage.objects.create(
            product=prod,
            image='https://res.cloudinary.com/dczihneby/image/upload/v1786900391/image_null_savm3g.png',
            is_thumbnail=True
        )
...
for i in range(1, 4):
    ...
    if not User.objects.filter(username=username).exists():
        u = User.objects.create_user(username=username, password='123', role='STAFF', address='Store', phone=f'090000010{i}', avatar='https://res.cloudinary.com/dczihneby/image/upload/v1786900184/default_avatar_jb1c0z.webp')
...
for i in range(20):
    ...
    order = Order.objects.create(
        ...
        total_amount=0,      # <-- Sửa lỗi: Cấp giá trị mặc định cho total_amount
        final_amount=0,
    )
    ...
    order.total_amount = order_total # <-- Cập nhật lại total_amount
